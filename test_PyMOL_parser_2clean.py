...
single_line_input = '1enr all zn,mn'

# User options
NMR = 1                 # 0/1: consider/discard NMR structures
res_threshold = 3       # resolution cut-off for apo chains (angstrom), condition is '<='
lig_free_sites = 1      # ensure that corresponding apo sites are free of other known ligands in addition to specified ligands

# Internal variables
job_id = '001'
overlap_threshold = 100  # % of overlap between apo and holo chain (w UniProt numbering), condition is '>='
ligand_scan_radius = '5' # angstrom radius to look around holo pocket
apo_chain_limit = 999    # limit number of apo chains to consider when aligning (faster)
...
nolig_resn = "ALA CYS ASP GLU PHE GLY HIS ILE LYS LEU MET ASN PRO GLN ARG SER THR VAL TRP TYR HOH".split()
...
if not user_chains == 'ALL':
    ...
    print('Input structure & chains:\t', user_structchains)
else:
...
# Discarded chains (format: structchain  discard_reason)
discarded_chains = list()

# Path for SIFTS file(s)
...
# Load normal SIFTS dictionary as dict_SIFTS
...
# Load dictionary with reverse_SIFTS (SPnum) as dict_rSIFTS
print('Loading reverse_SIFTS dictionary')
...
for user_structchain in user_structchains:
    uniprot_id = dict_SIFTS[user_structchain]
    for i in dict_rSIFTS[uniprot_id]:
        candidate_structchain = i.split(' ')[0] #pass structchain to variable
        if candidate_structchain[:4] != user_structchain[:4]: #discard same structure with user input (holo)
            ...
            for candidate in structchains_num:
        ...
        for i in values: # iterate over the values in each key, i = struct/chain combo
            if i.split()[0] == user_structchain:
                ...
                for candidate in values:
                    ...
                    if candidate.split()[0][:4] != structchain[:4]: # discard any structure same with query
                        ...
                        # Only keep positive overlap (negative means no overlap)
                        if percent > 0:
...
#print('Candidates over user threshold: ', dictApoCandidates)
...
## Apo candidate evaluation segment

# Put all structures for downloading into set
structs2download = set()
for key, values in dictApoCandidates.items():
    for i in values:    # iterate over the values in each key, i = struct/chain combo & SP_BEG SP_END etc
        ...
        structs2download.add(struct)
print('Total structures to download for next step: ', len(structs2download), '\n')

# Download/fetch/load the Apo candidate structures to specified directory [this should be replaced by fetch later #TODO]
for apo_candidate_structure in structs2download:
    try:
...
# Copy (shallow) the apo structure set with a more verbose name
apo_candidate_structs = structs2download.copy()
...
# Apply resolution, and method cutoff

# Parse structures to get resolution & method
#resolution line in mmCIF X-RAY: _refine.ls_d_res_high | in Cryo EM: _em_3d_reconstruction.resolution
...
for apo_candidate_struct in apo_candidate_structs:
    ...
    with gzip.open (apo_candidate_structPath, 'rt') as mmCIFin:
        ...
        try:
            #if resolution <= res_threshold or NMR == 1 and method == 'SOLUTION NMR':
...
print('Done\n')
print('Candidate chains over user-specified resolution [', res_threshold, 'Å ]: ', sum([len(dictApoCandidates_1[x]) for x in dictApoCandidates_1 if isinstance(dictApoCandidates_1[x], list)]), '\n')
...
for holo_structchain, apo_structchains in dictApoCandidates_1.items():
    ...
    if holo_struct in cmd.get_names('all'):
    ...
    '''
    '''
    if holo_struct in cmd.get_object_list('all'):
        pass
    else:    
        cmd.load(holo_struct_path)
    '''
    ...
    #ligands_selection = cmd.select('query_ligands', 'hetatm and name ' + ligand_names_bundle + ' and chain ' + holo_chain)
    ligands_selection = cmd.select('query_ligands', 'hetatm and resn ' + ligand_names_bundle + ' and chain ' + holo_chain)
    ...
    #print('Holo ligands positions for chain: ', holo_structchain,  holo_lig_positions[holo_structchain])
    ...
    for apo_structchain in apo_structchains:
        ...
        for ligand in holo_lig_positions[holo_structchain]:
            ...
            #print('Apo ligands detected in holo ligand sites: ', apo_structchain, apo_lig_names)
            #print('Checking if detected ligands are same with query ligands: ', holo_lig_names)
            
            #counter_lig = 0
            
            for i in apo_lig_names:
                if i in holo_lig_names:
        ...
        print(f'*holo (query) ligands: {holo_lig_names}\tapo ligands: {apo_lig_names}')
        if len(found_ligands) == 0:
            #print('*===> Apo chain', apo_structchain, ' clean of query ligands ', holo_lig_names)
            apo_holo_dict.setdefault(holo_structchain , []).append(apo_structchain + ' ' + str(round(aln_rms[0], 3)) + ' ' + str(round(aln_tm, 3)))
            print('PASS')
        else:
            #print('*apo chain', apo_structchain, ' includes query ligands ', found_ligands)
            print('FAIL')
    ...
    filename_pse = pathALN + '\\' + 'aln_' + holo_structchain + '_to_' + '_'.join(cmd.get_object_list('all and not ' + holo_struct)) + '.pse.gz'
