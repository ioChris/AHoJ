...
def load_precompiled_data_bin(workdir) -> PrecompiledData:
    pathSIFTS = workdir + '/SIFTS'
    fileSIFTSdict = pathSIFTS + '/pdb_chain_uniprot_dict.bin'        # regular SIFTS dictionary
    fileRSIFTS = pathSIFTS + '/pdb_chain_uniprot_REVERSE_SPnum.bin'  # reverse SIFTS (SPnum) dictionary
...
def load_precompiled_data(workdir) -> PrecompiledData:
    """Load pre-compiled files with UniProt PDB mapping generated by prepare.py"""
    res = load_precompiled_data_txt(workdir)
...
def process_query(query, workdir, args, data: PrecompiledData = None) -> QueryResult:
    ...
    :param data: pre-compiled data needed for computation, if not provided will be loaded on the fly
    :return:
    """
